...
#include <mutex>
#include <functional>
#include "Project.Common/rapidjson.hpp"

namespace script_engine {

	enum struct EnTargetSystem : std::uint64_t {
		SYSTEM_WINDOWS = 0,
		SYSTEM_LINUX = 1,
		SYSTEM_IOS = 3,
		SYSTEM_ANDROID = 4,
	...
	public:
		...
		virtual const std::uint64_t& ID() const = 0;
		virtual void ID(const std::uint64_t&) = 0;
		virtual std::string Identify() const = 0;
		virtual const std::uint64_t& TimeCompileBC() const = 0;
		virtual const std::uint64_t& TimeCompileCC() const = 0;
		virtual void TimeCompileCC(const std::uint64_t&) = 0;
		virtual void TimeCompileBC(const std::uint64_t&) = 0;
	...
	< Compile >
		...
		<arg key = "-triple" value = "x86_64-pc-windows-msvc19.28.29912" / >
